...
$pdf->Cell(
	100,		// bredde i mm (normalt 0)
	4,			// høyde i mm (normalt 0)
	utf8_decode(mb_strtoupper("Utestående per {$statusdato->format('d.m.Y')}", 'UTF-8')),
	$rammer,	// innramming? Boolsk eller 'LRTB' (normalt false)
	0,			// Neste markørposisjon: 0 = til høyre, 1 = til begynnelsen av neste linje, 2 = ned (normalt 0)
	'L',		// Justering: 'L', 'C' eller 'R' (normalt 'L')
	false		// bakgrunnsfarge eller ikke (normalt false)
...
foreach($kravsett as $krav) {
	$purringer = array_slice( $krav->hent('purringer'), 0, 6 );
	...
	if( $purringer ) {
		...
		foreach( $purringer as $purring ) {
			...
			$purrestreng .= $purring->hent('purredato')->format('d.m.Y');
		}
		$purrestreng = "Purret " . $purrestreng;
		$pdf->setY($Y);
		$pdf->setX($X+5);
		$pdf->Cell(120, 3, utf8_decode($purrestreng), $rammer, 0, 'L');
...
$pdf->setY($Y);
$pdf->setX($X);
$pdf->Cell(105, 3.5, utf8_decode("Totalt utestående:"), 'T', 0, 'L');
$pdf->setX($X + 105);
$pdf->Cell(15, 3.5, number_format($utestående, 2, ",", " "), 'T', 0, 'R');
...
//		Beløp
$pdf->setX(82.5);
$pdf->Cell(30, 5, "", $rammer, 0, 'R');

//		Kontrollsiffer
$pdf->setX(113.5);
$pdf->Cell(8, 5, "", $rammer, 0, 'L');
