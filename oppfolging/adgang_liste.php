...
?>
	...
	utførSletting = function(personid, område, leieforhold = 0){
		Ext.Ajax.request({
			waitMsg: 'Sletter...',
		...
		case "slett":
			$resultat = $this->mysqli->query(
				...
				AND adgang = '{$this->GET['adgang']}'"
				 . ($this->GET['adgang'] == 'beboersider' ? (" AND leieforhold = '{$this->GET['leieforhold']}'") : (""))
			);
			if($resultat['success'] = $this->mysqli->query("DrELETE FROM adganger WHERE personid = '{$this->GET['personid']}' AND adgang = '{$this->GET['adgang']}'" . ($this->GET['adgang'] == 'beboersider' ? (" AND leieforhold = '{$this->GET['leieforhold']}'") : ("")))) {
