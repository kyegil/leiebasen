				...
				foreach ($ledig as $leieobjektnr=>$info) {
					if($info['utleiegrad']['sum'] > 1) {
						...
						foreach ($info['utleiegrad']['kontrakter'] as $kontraktnr => $kontrakt){
							...
							echo "<a href=\"index.php?oppslag=leieforholdkort&id={$this->leieforhold($kontraktnr)}\">" . $this->brok($kontrakt['andel']) . " leies av " . $this->liste($this->kontraktpersoner($kontraktnr)) . " (" . date('d.m.Y', strtotime($b['data'][0]['fradato'])) . " - " . date('d.m.Y', $this->sluttdato($kontraktnr)) . ")</a><br />\n";
						}
						echo "<hr />\n";
					}
					else {
						...
						echo "<span class=\"bold green\">" . (($ledighet == '1') ? "" : "$ledighet av ") . "<a class=\"bold green\" href= \"index.php?oppslag=leieobjekt_kort&id=$leieobjektnr\">" . ($info['boenhet'] ? "bolig" : "lokale") . " nr. $leieobjektnr</a>: " . $this->etasjerenderer($info['etg']) . " " .(($info['navn']) ? ($info['navn'] . ", ") : $info['gateadresse']) . ", " . $info['beskrivelse'] . "</span><br />\nSiste leietaker(e): $sisteleietakere<br />";
						foreach ($info['utleiegrad']['kontrakter'] as $kontraktnr => $kontrakt){
							echo "<a href=\"index.php?oppslag=leieforholdkort&id={$this->leieforhold($kontraktnr)}\">" . $this->brok($kontrakt['andel']) . " leies av " . $this->liste($this->kontraktpersoner($kontraktnr)) . "</a><br />\n";
						}
						echo "<hr />\n";
					}
				}
			...
			foreach($endringer as $dato => $leieobjekter) {
				...
				foreach($leieobjekter as $leieobjekt => $endring) {
					...
					foreach($endring['slutt'] as $leieforhold) {
						...
						echo " (<a title=\"Åpne leieavtalen\" href=\"index.php?oppslag=leieforholdkort&id={$this->leieforhold($kontrakt['kontraktnr'])}\">leieavtale nr. {$kontrakt['kontraktnr']}</a>) flytter ut.<br />";
					}
					...
					foreach($endring['start'] as $leieforhold) {
						...
						$kontrakt = $kontrakt['data'][0];
						echo $this->liste($this->kontraktpersoner($leieforhold));
						echo " (<a title=\"Åpne leieavtalen\" href=\"index.php?oppslag=leieforholdkort&id={$this->leieforhold($kontrakt['kontraktnr'])}\">leieavtale nr. {$kontrakt['kontraktnr']}</a>) flytter inn.<br />";
					}					
					...
					foreach ($utleie['kontrakter'] as $kontraktnr => $kontrakt){
						echo ($kontrakt['andel'] < 1 ? ("<a href=\"index.php?oppslag=leieforholdkort&id={$this->leieforhold($kontraktnr)}\">" . $this->brok($kontrakt['andel']) . " leies av " . $this->liste($this->kontraktpersoner($kontraktnr)) . "</a><br />\n") : "");
					}
					echo "&nbsp;<br /></td></tr>";
				}
			}
