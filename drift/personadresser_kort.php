...
?>
	...
	var kort = new Ext.Panel({
		...
		html: [
			...
			(er_org ? ('Org nr: <b>' + personnr + '</b><br />\n') : ('Fødselsdato: <b>' + fødselsdato + ' ' + personnr + '</b><br />\n')),
		...
		buttons: [{
			handler: function() {
				window.location = "index.php?oppslag=profil_skjema&id=<?=$_GET['id']?>";
			},
			text: 'Vis profil for nettjenester'
		}, {
			handler: function() {
			...
			handler: function() {
				window.location = "index.php?oppslag=personadresser_skjema&id=<?=$_GET["id"]?>";
			},
			text: 'Rediger adressekortet'
		}]
	});
