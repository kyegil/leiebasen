...
public function __construct() {
	parent::__construct();
	$this->område['leieforhold'] = $this->hent('Leieforhold', $this->leieforhold( (int)@$_GET['id'] ) );
}
...
public function preHTML() {
	...
	if( !$leieforhold->hent('id') ) {
		...
		$this->område['leieforhold'] = $leieforhold;
...
?>
	...
	function visBrev(brev) {
		Ext.MessageBox.show({
			title: 'Brev',
			msg: "<div style=\"text-align: left;\">" + brev + "</div>",
			minWidth: 700
		});
	}


	var gråfremtid = function(value, metadata, record, rowIndex, colIndex, store) {
		nå = new Date();
	...
	var notatsett = Ext.create('Ext.data.Store', {
		model: 'Notat',
		...
		proxy: {
			type: 'ajax',
			simpleSortMode: true,
			url: "index.php?oppslag=leieforholdkort&id=<?php echo $leieforhold;?>&oppdrag=hentdata&data=notater",
			reader: {
				type: 'json',
				root: 'data',
				actionMethods: {
...
//			søkefelt
				...
				items: [
					{
						...
						handler: function() {
							window.open("index.php?oppslag=leieforhold_transaksjoner_utskrift&oppdrag=utskrift&id=<?php echo $leieforhold;?>");
						}
					},
					{
						text: 'Vis notater fra utleier',
						handler: function() {
							window.open("index.php?oppslag=notat_liste&id=<?php echo $leieforhold;?>");
						}
					}
				]
			})
	...
	var notatliste = Ext.create('Ext.grid.Panel', {
		itemId: 'notater',
		autoScroll: true,
		frame: false,
		features: [],
			...
			foreach( $betalinger as $betaling ) {
				foreach( $betaling->hent('delbeløp') as $delbeløp) {
					if(strval($delbeløp->krav) === strval($krav))
						$html .= "{$betaling->hent('dato')->format('d.m.Y')}: {$this->kr($delbeløp->beløp)}<br />";
				}
			}
...
<?
	...
	case "notater": {
		$resultat = $this->mysqli->arrayData(array(
			'source'		=> "{$tp}notater AS notater",
			'where'			=> "leieforhold = '{$leieforhold}'",
			'orderfields'	=> "dato, registrert"
		));
...
<?foreach( $leietakere as $leietaker ):?>
	<?php echo $leietaker->hent('navn');?><br />
<?php endforeach;?>
